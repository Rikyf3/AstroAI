...
def plot_batch(image, output, bkg, epoch, num_of_images=8, folder="./"):
    ...
    if num_of_images > 1:
        for i in range(num_of_images):
            axs[i, 0].imshow(stretch(image[i].cpu(), normalize=False))
            axs[i, 1].imshow(stretch(output[i].cpu(), normalize=False))
            axs[i, 2].imshow(stretch(bkg[i].cpu(), normalize=False))
    else:
        for i in range(num_of_images):
            axs[0].imshow(stretch(image[i].cpu(), normalize=False))
            axs[1].imshow(stretch(output[i].cpu(), normalize=False))
            axs[2].imshow(stretch(bkg[i].cpu(), normalize=False))
