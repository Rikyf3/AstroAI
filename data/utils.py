...
def median_abs_deviation(x, median_x):
    ...
    x = torch.median(x.reshape(3, -1), dim=1).values[:, None, None]
    return x
...
def plot_batch(image, output, bkg, epoch, num_of_images=8):
    ...
    for i in range(num_of_images):
        axs[i, 0].imshow(stretch(image[i].cpu(), normalize=False), interpolation="lanczos")
        axs[i, 1].imshow(stretch(output[i].cpu(), normalize=False), interpolation="bilinear")
        axs[i, 2].imshow(stretch(bkg[i].cpu(), normalize=False), interpolation="bilinear")
...
def lin_norm(x, med_=None, mad_=None):
    med_ = torch.median(x.reshape(3, -1), dim=1).values[:, None, None] if med_ is None else med_
